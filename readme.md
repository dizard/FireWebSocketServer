...
Многие решения предлагают в качестве транспорта http запрос что очень медленно либо очереди через редис, что тоже не быстро. Это нам не понравилось и вот что получилось))
Есть NET сервер с бинарным протокол обмена поддерживает как tcp соединение так и сокет соединение UNIX. Работает очень шустро и никаких транспортных расходов. 
